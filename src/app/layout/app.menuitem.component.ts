...
@Component({
    ...
    template: `
		<ng-container>
            <div *ngIf="root && item.visible !== false"    class="layout-menuitem-root-text text-white" [attr.href]="item.url">{{item.label}}</div>
            ...
            [ngClass]="item.class" [attr.target]="item.target" tabindex="0" pRipple [pTooltip]="item.label" [tooltipDisabled]="!(isSlim && root && !active)">
				<i [ngClass]="item.icon" class="text-white layout-menuitem-icon"></i>
				<span class="layout-menuitem-text text-white">{{item.label}}</span>
				<i class="pi pi-fw pi-angle-down layout-submenu-toggler text-white" *ngIf="item.items"></i>
			</a>
